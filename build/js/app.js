...
var _Mmenu = __webpack_require__(331);

var _Mmenu2 = _interopRequireDefault(_Mmenu);

var _InView = __webpack_require__(333);
...
var _Slick = __webpack_require__(336);

var _Slick2 = _interopRequireDefault(_Slick);

var _Magnific = __webpack_require__(338);

var _Magnific2 = _interopRequireDefault(_Magnific);

var _Wizard = __webpack_require__(340);
  ...
  Drupal.behaviors.govstrap = {
    attach: function attach(context, settings) {
      ...
      $(document).ready(function () {
        ...
        var toggleAffix = function toggleAffix(affixElement, wrapper, scrollElement) {
...
var _inView = __webpack_require__(334);
...
var InView = function () {
  ...
  _createClass(InView, [{
    ...
    value: function init() {
      ...
      var x = new _inView2.default('.main-banner.banner-animate').on('enter', function (el) {
        el.classList.add("animate");
      }).on('exit', function (el) {
        el.classList.remove("animate");
      });

      new _inView2.default('.single-image-left.banner-animate').on('enter', function (el) {
        el.classList.add("animate");
      }).on('exit', function (el) {
        el.classList.remove("animate");
      });

      new _inView2.default('.single-image-right.banner-animate').on('enter', function (el) {
        el.classList.add("animate");
      }).on('exit', function (el) {
        el.classList.remove("animate");
      });

      new _inView2.default('.fade-in-up').on('enter', function (el) {
...
var _magnificPopup = __webpack_require__(339);
...
function Wizard() {
  ...
  this.showQuestion = function (question) {
    var tpl = _this.getQuestionTpl();
    var optionTpl = _this.getOptionsTpl();
    ...
    for (var index in question.options) {
      var option = optionTpl.cloneNode(true);
      var data = question.options[index];
  ...
  this.setEventListener = function (option) {
    ...
    option.addEventListener('click', function (event) {
      var states = Array.from(option.parentElement.querySelectorAll('.icon'));

      for (var i in states) {
        states[i].classList.remove('active');
      }

      _this2.nextQuestion(option);
  ...
  this.getOptionsTpl = function () {
    var tpl = document.querySelector('.option-tpl').cloneNode(true);
  ...
  this.getQuestionTpl = function () {
    var tpl = document.querySelector('.item-tpl').cloneNode(true);
  ...
  this.nextQuestion = function (option) {
    var thisQuestion = globalData.filter(function (question) {
      return question.id === parseInt(option.getAttribute('data-parent'));
    })[0];
    var nextQuestion = globalData.filter(function (question) {
      return question.id === parseInt(option.getAttribute('data-goto'));
    })[0];

    thisQuestion = _this.resetActiveStates(thisQuestion, option);

    if (_this.isPreviousQuestion(thisQuestion)) {
      return _this.showPathway(thisQuestion, nextQuestion);
    }

    if (_this.pathwayExists(thisQuestion) && _this.hasPreviousSteps() && nextQuestion.type === 'question') {
      return _this.showPathway(thisQuestion, nextQuestion);
    }

    if (_this.isSameOptionGroup(option)) {
      return _this.changeSelectedOption(nextQuestion);
    }

    previousOption = option;

    _this.addPath(thisQuestion);

    _this.showQuestion(nextQuestion);

    _this.focusLastQuestion();
  };

  this.focusLastQuestion = function () {
  ...
  this.resetActiveStates = function (question, option) {
  ...
  this.clearActiveStates = function (question) {
    for (var i in question.options) {
      question.options[i].checked = false;
    }
  ...
  this.isPreviousQuestion = function (question) {
    return pathway.findIndex(function (item) {
      return item.id === question.id;
    }) > -1;
  };

  this.isSameOptionGroup = function (option) {
    return previousOption && option.getAttribute('data-parent') === previousOption.getAttribute('data-parent');
  };

  this.addPath = function (el) {
    currentParent = el;
    pathway.push(el);
  };

  this.pathwayExists = function (el) {
    return pathway.filter(function (path) {
      return path.id === el.id;
    }).length > 0;
  };

  this.showPathway = function (question, nextQuestion) {
    var index = pathway.indexOf(question);
    var newPathway = [];
    ...
    for (var _i in newPathway) {
      _this.showQuestion(newPathway[_i]);
    }

    _this.showQuestion(_this.clearActiveStates(nextQuestion));

    pathway = newPathway;
  };

  this.hasPreviousSteps = function () {
    return pathway.length > 1;
  };

  this.clearPathway = function () {
    pathway = [];
  };

  this.clearDisplayedQuestions = function () {
    var questions = Array.from(document.querySelectorAll('.question-wrap .question'));
  ...
  this.changeSelectedOption = function (next) {
    var els = document.querySelectorAll('.question-wrap .question');

    var el = els[els.length - 1];

    el.remove();

    _this.showQuestion(next);
  };
}
